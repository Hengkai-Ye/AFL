...
static inline u8 has_new_bits_2(u8* virgin_map) {
...
static u8 calibrate_case_2(char** argv, struct queue_entry* q, u8* use_mem,
                         u32 handicap, u8 from_queue) {
  ...
  if (q->exec_cksum) {
    ...
    memcpy(first_trace, trace_bits, MAP_SIZE);
    hnb = has_new_bits_2(virgin_bits);
    if (hnb > new_bits) new_bits = hnb;
...
static void perform_dry_run(char** argv, int dry_mode) {
  ...
  while (q) {
    ...
    close(fd);
    if(dry_mode){
      res = calibrate_case_2(argv, q, use_mem, 0, 1);
    }else{
...
int main(int argc, char** argv) {
  ...
  if (dry_mode){
    perform_dry_run(use_argv, dry_mode);
    exit(0);
  }
  perform_dry_run(use_argv, dry_mode);
  cull_queue();
